...
export default function DeleteProjectDialog({p, openModal,setOpenModal,deleteTask}
    : {openModal:boolean, setOpenModal:  React.Dispatch<React.SetStateAction<boolean>>,deleteTask: (p : string)=> void, p:string }
    ) {
            ...
            >
                <div className='sm:w-[450px] h-[200px]  p-4 flex gap-3 flex-col justify-between' >
                    ...
                    <main className='grow flex flex-col justify-between'>
                        <p>Are you sure you want to delete <span className='font-bold'>{p}</span>?</p>
                        <div className='self-end flex gap-4'>
                            <button className=' text-sm font-semibold  px-4 py-2 rounded bg-stone-100 hover:bg-stone-300' 
                            onClick={()=> {
                                console.log('delete cancel');
                                setOpenModal(false)}}>
                                Cancel
                            </button>
                            <button className='text-white text-sm font-semibold  px-4 py-2 rounded bg-red-500 hover:bg-red-700'
                             onClick={()=> {
                                deleteTask(p);
                                setOpenModal(false)
                             }}>
